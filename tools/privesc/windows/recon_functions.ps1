...
function grepdb () {
    ...
    $regex = '(.*(sql|mongo)(.|\n)*(localhost|127.0.0.1|0.0.0.0).*)|(.*(localhost|127.0.0.1|0.0.0.0)(.|\n)*(sql|mongo).*)'; get-childitem -depth 5 |% { $tmp = $_.FullName; get-content $_.FullName -Raw -erroraction 'silentlycontinue'} | select-string -pattern $regex -AllMatches | % {$_.Matches} | % {write-host -ForegroundColor Yellow "`n`n" $tmp "`n"; write-host $_.Value}
